...
def compute(s: str) -> int:
    ...
    space_to_be_freed = NEEDED_SPACE - (MAX_SPACE - root.get_size())

    folder_candidates = sorted([folder for folder in all_folders if folder.get_size(
    ) >= space_to_be_freed], key=lambda folder: folder.get_size())

    return folder_candidates[0].get_size()
